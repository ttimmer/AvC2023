...
total_sum = 0
games = """Game 1: 7 blue, 5 red; 10 red, 7 blue; 5 blue, 4 green, 15 red; 4 green, 6 red, 7 blue; 5 green, 8 blue, 4 red; 5 red, 4 blue, 3 green
...
game = games.splitlines()
game_numbers =0
calculatedPowers =0
for entry in game:
    xnotpossible = False
    gamenumber = entry.split(':')[0];
    gamesubsets = entry.split(':')[1];
    subset = gamesubsets.split(';')
    lowestBlue = 0
    lowestred = 0
    lowestGreen = 0

    for subgame in subset:

        subsets = subgame.split(', ')
        # Find and extract the amount of blue cubes
        blue_amount = 0
        red_amount = 0
        green_amount = 0

        for subsetted in subsets:
            count, color = subsetted.split()
            if color.lower() == 'blue':
                blue_amount = int(count)
                if blue_amount>lowestBlue:
                    lowestBlue = blue_amount
            elif color.lower() == 'red':
                red_amount = int(count)
                if red_amount>lowestred:
                    lowestred = red_amount
            elif color.lower() == 'green':
                green_amount = int(count)
                if green_amount>lowestGreen:
                    lowestGreen= green_amount;


    calculatedPowers += lowestGreen*lowestBlue*lowestred

    if not xnotpossible:
        game_numbers += [int(match.group(1)) for match in re.finditer(r'Game (\d+)', gamenumber)][0]

print(game_numbers)
print(calculatedPowers)
