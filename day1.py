...
3nine6five1
    ...
    add = 0;
    for line in lines:
        if not((find_calibration_values(line)) == None):
            add += find_calibration_values(line)
            print(f"Value is {find_calibration_values(line)}")
    print(add)
...
def find_calibration_value(line):
    ...
    if line.strip():
        ...
        # If both digits are found, combine to form a two-digit number
        if first_digit is not None and last_digit is not None:
            calibration_value = int(str(first_digit) + str(last_digit))
            return calibration_value

    # Return None for empty lines or lines without digits
    return None


def find_calibration_values(line):
    # Function to convert spelled-out digits to numerical digits
    # Find the real first and last digits on each line
    print(line)
    return(find_calibration_value(convert_to_digit(line)))

def convert_to_digit(word):
    digit_mapping = { 'twone':'21','eightwo' : '82','zeroone':'01','zero': '0', 'oneight':'18' ,'one': '1'  ,'two': '2', 'threeight':'38','three': '3', 'four': '4', 'fiveight':'58', 'five': '5', 'six': '6', 'seven': '7', 'eight': '8', 'nine': '9'}
    for x in digit_mapping.keys():
        if x in word:
            word = word.replace(x,digit_mapping[x])
