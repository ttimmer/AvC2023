...
def check_poker_hand_with_jokers(hand):
    counts = Counter(hand)
    counts.pop('J', None)
    if len(counts)>0:
        replace_by = max(counts, key=counts.get)
    else:
        replace_by = "J"
    replaced_hand = hand.replace('J',replace_by)

    print(f"hand: {hand} replaced {replaced_hand} {check_poker_hand(hand)} to {check_poker_hand(replaced_hand)} replace by {replace_by}")
    return check_poker_hand(replaced_hand)
...
def perform_solution(input):
    ...
    five_kind = [card for card in sorted_hands if card.rank == "Five of a kind"]
    ...
    for hand in hands:
        i+=1
        print(f"Hand {hand.cards} with type = {hand.rank} bid {hand.bid} entry{i}")
        totalsum += int(hand.bid)*i
    return totalsum
def perform_solution2(input):
    inputlines = input.splitlines()
    cardArray = []
    for entry in inputlines:
        init_card = Card(0, entry.split(' ')[0], entry.split(' ')[1], check_poker_hand_with_jokers(entry.split(' ')[0]))
        cardArray.append(init_card)
    sorted_hands = sorted(cardArray, key=lambda Card: (Card.ranknumber))
    hands = []
    high_cards = [card for card in sorted_hands if card.rank == "High card"]
